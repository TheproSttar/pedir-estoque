...
import { ActionRowBuilder, ButtonBuilder, ButtonStyle, ChannelType, EmbedBuilder, StringSelectMenuBuilder, TextChannel , time} from "discord.js";
...
createResponder({
    ...
    async run(interaction) {
        ...
        const canalLogsChannel = await interaction.guild?.channels.fetch(canalLogs) as TextChannel | null;
        ...
        if (canalLogsChannel && canalLogsChannel.isTextBased()) {
            const embed = new EmbedBuilder()
                ...
                .setDescription(
                    `*Este foi um pedido de Reestock de um produto, ou para um novo produto*\n\n` +
        ...
        const rowSelectMenu = new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(selectmenu)
