...
const cooldowns = new Map(); // guarda userId -> timestamp
...
createResponder({
    ...
    async run(interaction) {
        ...
        if (customId === "sair-da-thread") {
            ...
            const randomTchau = msgtchau[Math.floor(Math.random() * msgtchau.length)];
        ...
        if (customId === "chamar-equipe") {
            ...
            if (lastUsed && now - lastUsed < cooldownTime) {
                ...
                let niceTime = "";
                if (minutes > 0)
                    niceTime += `${minutes} minuto${minutes > 1 ? "s" : ""}`;
                if (minutes > 0 && seconds > 0)
                    niceTime += " e ";
                if (seconds > 0)
                    niceTime += `${seconds} segundo${seconds > 1 ? "s" : ""}`;
